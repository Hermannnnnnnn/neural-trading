  ...
  def calculate_percentual_change(self, field: str ='Close', steps: int =1) -> None:
    ...
    if field not in column_headers:
        ...
        for ind in range(steps):
          ...
          self.df_prices[f'perc_ch_{ind}_steps'] = ( self.df_prices[field].shift(-ind) - self.df_prices[field])/( self.df_prices[field])*100
    self.df_prices.dropna(how='any', inplace=True)
    return
  
  def bucketting_prices(self, bins: list =[-100,-2,-1,1,2,100],labels: list = [-2,-1,0,1,2]) -> None:# labels=['Very bad', 'bad', 'neutral', 'good', 'very good']):
      """
      ...
      def find_first_not_stop_loss_index():
        for index in range(len(bins)):
            if bins[index] > self.stop_loss:
                return index
        raise ValueError('There must be a bin where the max boundary is larger than the stop_loss')
      
      first_not_stop_loss_index = find_first_not_stop_loss_index()

      def min_max_function(row):
        # print(row)
        value = 0.0
        for index, values in row.items():
          if values >= bins[first_not_stop_loss_index]:
            value = max(value,values)
          elif values <= self.stop_loss:#row[index]
            if value >= bins[first_not_stop_loss_index]:
               return value
            else:
               return values
        return value
...
#       # self.df_prices[f'perc_ch_{steps}_steps_binned'] = pd.cut(self.df_prices[columns].apply(min_max_function, axis=1), bins=bins,labels=labels)#self.df_prices[columns].max(axis=1)
  ...
  def gimme_ema(self,ema_periods=26):
      self.quotes_list = [
                          Quote(d,o,h,l,c,v) 
                          for d,o,h,l,c,v 
                          in zip(self.df_prices['Date'], self.df_prices['Open'], self.df_prices['High'], self.df_prices['Low'], self.df_prices['Close'], self.df_prices['Volume'])
      ...
      # self.df_prices[f'macd_{fast_periods}_{slow_periods}_{signal_periods}'] = [x.macd for x in indicators.get_macd(self.quotes_list, fast_periods=fast_periods, slow_periods=slow_periods, signal_periods=signal_periods)]
      return  [x.ema for x in indicators.get_ema(self.quotes_list, lookback_periods=ema_periods)]
